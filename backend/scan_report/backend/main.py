...
class EnhancedMedicalAnalyzer:
    ...
    def _classify_medical_condition(self, patterns, body_part, condition_scores=None):
        ...
        risk_score = 0.0
        # Mass contribution: diminishing returns after 3
        ...
        # Add condition-specific contributions
...
async def generate_pdf_report(
    ...
    try:
        ...
        # Title
        story.append(Paragraph("DARKMED AI - MEDICAL IMAGE ANALYSIS REPORT", title_style))
        ...
        story.append(Paragraph(f"<b>Analysis Date:</b> {analysis.get('analysis_timestamp', '2024-01-15')}", normal_style))
        ...
        if 'medical_findings' in analysis:
            ...
            findings_data = [
                ['Finding', 'Details'],
                ['Potential Masses', str(findings.get('potential_masses', 0))],
        ...
        if 'doctor_recommendations' in analysis:
            ...
            if recommendations.get('risk_based_recommendations'):
                story.append(Paragraph("<b>Risk-Based Actions:</b>", normal_style))
                for rec in recommendations['risk_based_recommendations']:
                    story.append(Paragraph(f"• {rec}", normal_style))
                story.append(Spacer(1, 6))
            
            if recommendations.get('medical_recommendations'):
                story.append(Paragraph("<b>Medical Actions:</b>", normal_style))
                for rec in recommendations['medical_recommendations']:
                    story.append(Paragraph(f"• {rec}", normal_style))
                story.append(Spacer(1, 6))
            
            if recommendations.get('general_recommendations'):
                story.append(Paragraph("<b>General Actions:</b>", normal_style))
                for rec in recommendations['general_recommendations']:
                    story.append(Paragraph(f"• {rec}", normal_style))
                story.append(Spacer(1, 12))
        ...
        if 'quality_assessment' in analysis:
            ...
            quality_data = [
                ['Aspect', 'Rating'],
                ['Overall', quality.get('overall_rating', 'N/A')],
        ...
        disclaimer_text = analysis.get('disclaimer', 'This is a preliminary AI-assisted analysis for licensed clinicians only. This is not a diagnosis and should not replace professional medical judgment.')
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename={filename}_medical_report.pdf"}
        )
